 ...
 */
class ErrorController extends Cms_Controller_ActionAbstract
{ 
    /**
    ...
    { 
        ...
        if ($this->getInvokeArg('errorview') && $this->getInvokeArg('errorview') != 'error' && getenv('APPLICATION_ENV') == 'development') {
	...
	if ($errors->exception->getCode() == 401) {
            $this->view->breadcrumb('Błąd 401');
            $this->renderScript('error/401.phtml');
	} else if($errors->exception->getCode() == 404 || getenv('APPLICATION_ENV') != 'development') {
            $this->renderScript('error/404.phtml');
            $this->view->breadcrumb('Błąd 404');
	}
    } 
}
